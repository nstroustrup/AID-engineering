...
We check which samples have a low content of mRNA.
```{r}
...
We only consider transcripts that have at least 5 counts in more than 50% of the samples. We end up with:
```{r echo=FALSE}
...
```{r}
#look at effect of TIR1 variant using the wrong type of cross-term model
if (0){
...
results_aux_ind2_x = cbind(results_aux_ind_x[[1]],results_aux_ind_x[[2]],results_aux_ind_x[[3]],results_aux_ind_x[[4]],results_aux_ind_x[[5]],results_aux_ind_x[[6]])
...
```{r}
#recalc the effects auxin in the presence of the double (e.g the "on target" effects) *only for wildtype
#NOTE: THiS APPROACH ASSUMES THE effects of auxin are *additive* to the effects of TIR1, which they are *not* because adding TIR1 appears to 
#protect the animals against some effects of Auxin.  
#The result is that these "on-target" effects have baked into them the effects of auxin that are removed by adding TIR1
#these effects are *different* between the different TIR1 variants and so they artifically inflate the differences between variants.
if (0){
annots_ontarget = annotations_aie[Promoter=="eft3p" & Tir1=="WT"&Day==1&daf2AID==1,]
genes_to_exclude = c("TIR1-DERNBERG")
...
annots_ontarget$label = factor(ifelse(annots_ontarget$Auxin,"auxin","none"))
annots_ontarget$label = relevel(annots_ontarget$label,"none")
dds_aux_ontarget<- DESeqDataSetFromMatrix(countData = counts_ontarget,
                                     colData = annots_ontarget,
                                     design = ~ label)
sizeFactors(dds_aux_ontarget) <- nf_ontarget # use scran normalization
dds_aux_ontarget <- estimateDispersions(dds_aux_ontarget) # estimate overdispersion parameter
dds_aux_ontarget <- nbinomWaldTest(dds_aux_ontarget) # run NB GLM and do Wald Test on coefficients

results_aux_ontarget <- lapply(resultsNames(dds_aux_ontarget), function(i) {
  r <- results(dds_aux_ontarget, name = i)
  r <- as.data.table(r)
  colnames(r) <- paste0(i, "_", colnames(r))
  r
})

results_aux_ontarget_long <- rbindlist(lapply(resultsNames(dds_aux_ontarget), function(i) {
  r <- results(dds_aux_ontarget, name = i)
  r <- as.data.table(r)
...
results_aux_ontarget_long = results_aux_ontarget_long[covariate != "Intercept",]
results_aux_ontarget_long[,group:=substring(covariate,7,nchar(covariate)-8)]
results_aux_ontarget_long[,label:=group]
}

```

```{r}
#recalc the effects auxin in the presence of the double (e.g the "on target" effects) *across all three
#NOTE: THiS APPROACH ASSUMES THE effects of auxin are *additive* to the effects of TIR1, which they are *not* because adding TIR1 appears to 
#protect the animals against some effects of Auxin.  
#The result is that these "on-target" effects have baked into them the effects of auxin that are removed by adding TIR1
#these effects are *different* between the different TIR1 variants and so they artifically inflate the differences between variants.
if (0){
annots_ontarget = annotations_aie[Promoter=="eft3p" &Day==1&daf2AID==1,]
genes_to_exclude = c("TIR1-DERNBERG")
counts_ontarget= counts[ rownames(counts)%!in%genes_to_exclude,annots_ontarget$Sample]
nf_ontarget <- calculateSumFactors(counts_ontarget)
...
annots_ontarget$label = factor(ifelse(annots_ontarget$Auxin,"auxin","none"))
annots_ontarget$label = relevel(annots_ontarget$label,"none")
dds_aux_ontarget_all3<- DESeqDataSetFromMatrix(countData = counts_ontarget,
                                     colData = annots_ontarget,
                                     design = ~ label)
sizeFactors(dds_aux_ontarget_all3) <- nf_ontarget # use scran normalization
dds_aux_ontarget_all3 <- estimateDispersions(dds_aux_ontarget_all3) # estimate overdispersion parameter
dds_aux_ontarget_all3 <- nbinomWaldTest(dds_aux_ontarget_all3) # run NB GLM and do Wald Test on coefficients

results_aux_ontarget_all3 <- lapply(resultsNames(dds_aux_ontarget_all3), function(i) {
  r <- results(dds_aux_ontarget_all3, name = i)
  r <- as.data.table(r)
  colnames(r) <- paste0(i, "_", colnames(r))
  r
})

results_aux_ontarget_long_all3 <- rbindlist(lapply(resultsNames(dds_aux_ontarget_all3), function(i) {
  r <- results(dds_aux_ontarget_all3, name = i)
  r <- as.data.table(r)
  r[, GeneName := rownames(counts_ontarget)]
  r$covariate = i
...
results_aux_ontarget_long_all3 = results_aux_ontarget_long_all3[covariate != "Intercept",]
results_aux_ontarget_long_all3[,group:=substring(covariate,7,nchar(covariate)-8)]
}
...
```{r}
if (0){
#recalc the effects auxin in the presence of the double (e.g the "on target" effects) *Differences between variants
annots_ontarget = annotations_aie[Promoter=="eft3p" & Auxin==T & Day==1& daf2AID==1,]
genes_to_exclude = c("TIR1-DERNBERG")
counts_ontarget= counts[ rownames(counts)%!in%genes_to_exclude,annots_ontarget$Sample]
nf_ontarget <- calculateSumFactors(counts_ontarget)


annots_ontarget$Tir1 = relevel(factor(annots_ontarget$Tir1),"WT")
dds_aux_ontarget_variant_effects<- DESeqDataSetFromMatrix(countData = counts_ontarget,
                                     colData = annots_ontarget,
                                     design = ~ Tir1)
sizeFactors(dds_aux_ontarget_variant_effects) <- nf_ontarget # use scran normalization
dds_aux_ontarget_variant_effects <- estimateDispersions(dds_aux_ontarget_variant_effects) # estimate overdispersion parameter
dds_aux_ontarget_variant_effects <- nbinomWaldTest(dds_aux_ontarget_variant_effects) # run NB GLM and do Wald Test on coefficients

results_aux_ontarget_variant_effects <- lapply(resultsNames(dds_aux_ontarget_variant_effects), function(i) {
  r <- results(dds_aux_ontarget_variant_effects, name = i)
  r <- as.data.table(r)
  colnames(r) <- paste0(i, "_", colnames(r))
  r
})

results_aux_ontarget_long_variant_effects <- rbindlist(lapply(resultsNames(dds_aux_ontarget_variant_effects), function(i) {
  r <- results(dds_aux_ontarget_variant_effects, name = i)
  r <- as.data.table(r)
  r[, GeneName := rownames(counts_ontarget)]
  r$covariate = i
...
results_aux_ontarget_long_variant_effects = results_aux_ontarget_long_variant_effects[covariate != "Intercept",]
results_aux_ontarget_long_variant_effects[,group:=substring(covariate,7,nchar(covariate)-8)]
}
```
```{r}
if (0){
...
if (0){
library(ggrastr)
#compare TIR1 effects to variant_specific effects

res = rbind(results_aux_ontarget_long_all3,results_aux_ontarget_long_variant_effects)

axis_lims = c(-2.5,4)
res[log2FoldChange<axis_lims[1],log2FoldChange:=axis_lims[1]]
res[log2FoldChange>axis_lims[2],log2FoldChange:=axis_lims[2]]


res_l_FC = dcast(res,GeneName~covariate,value.var="log2FoldChange")
names(res_l_FC)[2:4] = paste0(names(res_l_FC)[2:4],".log2FC")

res_l_padj = dcast(res,GeneName~covariate,value.var="padj")
names(res_l_padj)[2:4] = paste0(names(res_l_padj)[2:4],".padj")

res_l_a = merge(res_l_FC,res_l_padj,by="GeneName")
res_l_a[,significance:=ifelse(label_auxin_vs_none.padj<.001 & Tir1_F79A_vs_WT.padj < .001,"Both",
                      ifelse(label_auxin_vs_none.padj<.001,"All three variants",
                             ifelse(Tir1_F79A_vs_WT.padj<.001,"TIR1[79A]-specific","Neither")))]
res_l_a = res_l_a[!is.na(significance)&(significance!="Neither" | Tir1_F79G_vs_WT.padj < .001),]

colors = c("black","#88EEFF","red")
sizes = c(2,2,2)
names(colors) = c("All three variants","TIR1[79A]-specific","Both")
names(sizes) = c("All three variants","TIR1[79A]-specific","Both")
  
cr =cor(res_l_a$label_auxin_vs_none.log2FC,res_l_a$Tir1_F79A_vs_WT.log2FC)
g1=ggplot(res_l_a,aes(label_auxin_vs_none.log2FC,Tir1_F79A_vs_WT.log2FC,color=significance)) + 
  geom_smooth(method="lm",lwd=1,color="dark grey",se=F)+
  geom_hline(yintercept=0,wd=1,color="grey",lty=2)+
  geom_vline(xintercept=0,wd=1,color="grey",lty=2)+
  geom_point_rast(aes(fill=significance,size=significance),pch=21,color="black",alpha=.5,raster.dpi=600) +  geom_abline(intercept=0,slope=1,lty=2,col="dark gray")+xlab("Common effect across all variants\n(log2 fold-change)")+ylab("TIR1[79A]-specific")+ scale_fill_manual(values=colors)+scale_size_manual(values=sizes)+theme(legend.position="none")+xlim(axis_lims[1],axis_lims[2])+ylim(axis_lims[1],axis_lims[2])+
  annotate("text",label=paste0("rho=",round(cr,2)),x=axis_lims[1]*3/4,y=axis_lims[2]*3/4)
  

res_l_a = merge(res_l_FC,res_l_padj,by="GeneName")
res_l_a[,significance:=ifelse(label_auxin_vs_none.padj<.001 & Tir1_F79G_vs_WT.padj < .001,"Both",
                      ifelse(label_auxin_vs_none.padj<.001,"All three variants",
                             ifelse(Tir1_F79G_vs_WT.padj<.001,"TIR1[79G]-specific","Neither")))]
res_l_a = res_l_a[!is.na(significance)&(significance!="Neither" | Tir1_F79A_vs_WT.padj < .001),]

names(colors) = c("All three variants","TIR1[79G]-specific","Both")
names(sizes) = c("All three variants","TIR1[79G]-specific","Both")
  

cr =cor(res_l_a$label_auxin_vs_none.log2FC,res_l_a$Tir1_F79G_vs_WT.log2FC)
g2=ggplot(res_l_a,aes(label_auxin_vs_none.log2FC,Tir1_F79G_vs_WT.log2FC,color=significance)) + 
  geom_smooth(method="lm",lwd=1,color="dark grey",se=F)+
  geom_hline(yintercept=0,wd=1,color="grey",lty=2)+
  geom_vline(xintercept=0,wd=1,color="grey",lty=2)+
  geom_point_rast(aes(fill=significance,size=significance),pch=21,color="black",alpha=.5,raster.dpi=600) +  geom_abline(intercept=0,slope=1,lty=2,col="dark gray")+xlab("Common effect across all variants\n(log2 fold-change)")+ylab("TIR1[79G]-specific")+ scale_fill_manual(values=colors)+scale_size_manual(values=sizes)+theme(legend.position="none")+xlim(axis_lims[1],axis_lims[2])+ylim(axis_lims[1],axis_lims[2])+
  annotate("text",label=paste0("rho=",round(cr,2)),x=axis_lims[1]*3/4,y=axis_lims[2]*3/4)


res_l_a = merge(res_l_FC,res_l_padj,by="GeneName")
res_l_a[,significance:=paste0(ifelse(label_auxin_vs_none.padj<.001,"All three variants ",
                      paste0(ifelse(Tir1_F79G_vs_WT.padj<.001,"TIR1[79G]-specific ",""),
                       ifelse((label_auxin_vs_none.padj<.001 | Tir1_F79G_vs_WT.padj<.001) & Tir1_F79A_vs_WT.padj < .001 , " & ",""),
                         ifelse(Tir1_F79A_vs_WT.padj<.001,"TIR1[79A]-specific",""))))]
res_l_a = res_l_a[!grepl("NA",significance)&significance!="",]

res_l_a[,variant_specific.log2FC :=ifelse(significance=="TIR1[79A]-specific",Tir1_F79A_vs_WT.log2FC,
                                   ifelse(significance=="TIR1[79G]-specific",Tir1_F79G_vs_WT.log2FC,
                                          (Tir1_F79G_vs_WT.log2FC+Tir1_F79A_vs_WT.log2FC)/2))]

colors = c("black","#64C5EB","#E84D8A","#FEB326")
names(colors) = c("All three variants ","TIR1[79A]-specific","TIR1[79G]-specific ","TIR1[79G]-specific  & TIR1[79A]-specific")
sizes = c(2,2,2,2)
names(colors) = c("All three variants ","TIR1[79A]-specific","TIR1[79G]-specific ","TIR1[79G]-specific  & TIR1[79A]-specific")
  
unique(res_l_a[,.(significance,colors[significance])])
cr =cor(res_l_a$label_auxin_vs_none.log2FC,res_l_a$variant_specific.log2FC)
g3=ggplot(res_l_a,aes(label_auxin_vs_none.log2FC,variant_specific.log2FC,color=significance)) + 
  geom_smooth(method="lm",lwd=1,color="dark grey",se=F)+
  geom_hline(yintercept=0,wd=1,color="grey",lty=2)+
  geom_vline(xintercept=0,wd=1,color="grey",lty=2)+
  geom_point_rast(aes(fill=significance,size=significance),pch=21,color="black",alpha=.7,raster.dpi=600) +  
  geom_abline(intercept=0,slope=1,lty=2,col="dark gray")+xlab("Common effect across all variants\n(log2 fold-change)")+
  ylab("TIR1[79G] or TIR1[79A]\n specific effect\n(log2 fold-change)")+ 
  scale_fill_manual(values=colors)+
  scale_size_manual(values=sizes)+xlim(axis_lims[1],axis_lims[2])+ylim(axis_lims[1],axis_lims[2])+
  annotate("text",label=paste0("rho=",round(cr,2)),x=axis_lims[1]*3/4,y=axis_lims[2]*3/4)
  
ggarrange(plotlist=list(g1,g2,g3),widths=c(1,1,2.2),nrow=1)
ggsave("general_vs_specific_TIR1_ontarget_effects.pdf",width=15,height=4)
}
...
```{r}
#compare the effects of different variants to each other, accounting for the "off-target" effects of compounds
...
TIR1_reference_groups = c("WT","none")  #the first reference group lets us calculate p values for differences between WT and other variants
...
multiple_regs = lapply(TIR1_reference_groups,function(TIR1_reference_group){
   ...
   #                                                      "active","not_active")),ref="active")
    ...
    #print((annots_ontarget[,.(AuxinType, Tir1_variant,TIR1_actived)]))
    #stop()
    #dds_aux_other_variants_vs_F79= DESeqDataSetFromMatrix(countData = counts_ontarget,
    #                                     colData = annots_ontarget,
     #                                    design = ~ AuxinType+Tir1_variant*TIR1_actived
    #                                     )
...
#this is good and important because there are effects of the compounds that are removed by adding TIR1, and modelling B_compound
#as consistent across all conditions excludes these.
#BUT THIS IS BAD BECAUSE IT MODELS the TIR1 effects as consistent with or without compound!  We need to run the cross term model as done above.
if (0){
TIR1_variants = c("WT","F79G","F79A")
AID_repeats = c(1,3);

multiple_regs = lapply(TIR1_variants,function(TIR1_variant){
    res = lapply(AID_repeats,function(AID_repeat){
    compound = annotations_aie[Tir1==TIR1_variant,AuxinType]
    
    annots_ontarget = annotations_aie[Biological_replicate==1 &Day==1& AuxinType%in% c(compound,"wo") & (Promoter == "eft3p" & Tir1 == TIR1_variant & daf2AID==AID_repeat | Strain=="QZ0"),]
    ...
    annots_ontarget$AuxinType = relevel(factor(annots_ontarget$AuxinType),ref="wo")
    #annots_ontarget$Tir1_present = relevel(factor(ifelse(!is.na(annots_ontarget$Tir1),"TIR1","none")),ref="none")
    annots_ontarget$Tir1_variant = relevel(factor(ifelse(!is.na(annots_ontarget$Tir1),annots_ontarget$Tir1,"none")),ref="none")
    
    
    #View(annots_ontarget[,.(AuxinType, Tir1_variant,Genotype)])
    #stop()
    dds_aux_other_variants_vs_F79= DESeqDataSetFromMatrix(countData = counts_ontarget,
                                         colData = annots_ontarget,
                                         design = ~ AuxinType*Tir1_variant
                                         )
    sizeFactors(dds_aux_other_variants_vs_F79) <- nf_ontarget # use scran normalization
    dds_aux_other_variants_vs_F79 <- estimateDispersions(dds_aux_other_variants_vs_F79) # estimate overdispersion parameter
    dds_aux_other_variants_vs_F79 <- nbinomWaldTest(dds_aux_other_variants_vs_F79) # run NB GLM and do Wald Test on coefficients
    ...
    annots_activated_TIR1 = annots_ontarget[AuxinType==compound) & (Promoter == "eft3p" & Tir1 == TIR1_variant & daf2AID==AID_repeat | Strain=="QZ0"),]
...
names(multiple_regs) = TIR1_variants;
...
if (0){ #old bad way of doing it
#plot the 'on-target' effects of the activated auxin system PROMOTER EFFECT and DEGRON repeat
tmp = results_aux_promoter_wide
sig = .01
mag_cutoff =  log2(2)

results_aux_promoter_wide_melt_logFC = melt(tmp,id.vars=c("GeneName","Tir1_promoter_wo.0xAID.logFC"),measure.vars=c("Tir1_promoter_eif3Bp.1xAID.logFC","Tir1_promoter_eft3p.3xAID.logFC"))

results_aux_promoter_wide_melt_logFC[,value := -value]  #because the regression is set up relative to eft3p and we need to flip the signs so the effects are WT->eft3p::tir1

results_aux_promoter_wide_melt_logFC[,TIR1:=substring(as.character(variable),0,nchar(as.character(variable))-6)]

results_aux_promoter_wide_melt_padj=
melt(tmp,id.vars=c("GeneName","Tir1_promoter_wo.0xAID.padj"),measure.vars=c("Tir1_promoter_eif3Bp.1xAID.padj","Tir1_promoter_eft3p.3xAID.padj"))
results_aux_promoter_wide_melt_padj[,TIR1:=substring(as.character(variable),0,nchar(as.character(variable))-5)]

results_aux_promoter_wide_melt_both = merge(results_aux_promoter_wide_melt_logFC,results_aux_promoter_wide_melt_padj,by=c("GeneName","TIR1"),suffixes=c(".logFC",".padj"))

results_aux_promoter_wide_melt_both[,significance := ifelse(Tir1_promoter_wo.0xAID.padj<sig & value.padj<sig & 
                                                                   abs(Tir1_promoter_wo.0xAID.logFC) >= mag_cutoff & abs(value.logFC) >= mag_cutoff,
                                                                 "Both",
                                                                 ifelse(Tir1_promoter_wo.0xAID.padj<sig & abs(Tir1_promoter_wo.0xAID.logFC)>=mag_cutoff,"eft-3",
                                                                        ifelse(value.padj<sig & abs(Tir1_promoter_wo.0xAID.logFC+value.logFC)>=mag_cutoff,substring(TIR1,15),
                                                                               "Neither")))]

colors = c("black","black","#FCEA64","#98cF6F","white")
names(colors) = c("Both","eft-3","eif3Bp.1xAID","eft3p.3xAID" ,"Neither")
sig_genes =  tmp[Tir1_promoter_eif3Bp.1xAID.padj < sig | Tir1_promoter_wo.0xAID.padj < sig | Tir1_promoter_eft3p.3xAID.padj<sig,GeneName]
results_aux_promoter_wide_melt_both = results_aux_promoter_wide_melt_both;#[GeneName %in% sig_genes,]
axis_lims = range(c(-results_aux_promoter_wide_melt_both$Tir1_promoter_wo.0xAID.logFC,results_aux_promoter_wide_melt_both$value.logFC))
#unique(res_l_a[,.(significance,colors[significance])])
#cr =cor(res_l_a$label_auxin_vs_none.log2FC,res_l_a$variant_specific.log2FC)
  
if (0){
g_a_resid=ggplot(results_aux_promoter_wide_melt_both[TIR1=="Tir1_variant_F79A" ,],aes(Tir1_promoter_wo.0xAID.logFC,value.logFC,color=significance)) + 
 
  geom_smooth(method="lm",se=F,aes(color=NULL),color="#537D5D")+
  geom_hline(yintercept=0,color="grey",lty=2)+
  geom_vline(xintercept=0,color="grey",lty=2)+ 
  geom_point_rast(data=results_aux_promoter_wide_melt_both[TIR1=="Tir1_variant_F79A"  & significance=="Neither"], aes(fill=significance),pch=21,color="black",alpha=.3,raster.dpi=600)+
  geom_point_rast(data=results_aux_promoter_wide_melt_both[TIR1=="Tir1_variant_F79A"  & significance!="Neither"], aes(fill=significance),pch=21,color="black",alpha=.7,raster.dpi=600) +  
  geom_abline(intercept=0,slope=1,lty=2,col="dark gray")+ 
 # geom_abline(intercept=0,slope=-1,lty=2,col="dark gray")+
  ylab("Differential TIR1[F79A] effect (log2 fold-change)")+
  xlab("TIR1[F79] effect (log2 fold-change)")+ 
  scale_fill_manual(values=colors)+
  scale_color_manual(values=colors)+
  xlim(axis_lims[1],axis_lims[2])+ylim(axis_lims[1],axis_lims[2])+
...
g_g_resid=ggplot(results_aux_promoter_wide_melt_both[TIR1=="Tir1_variant_F79G",],aes(Tir1_variant_none.logFC,value.logFC,color=significance)) + 
  geom_smooth(method="lm",se=F,aes(color=NULL),color="#537D5D")+
  geom_hline(yintercept=0,color="grey",lty=2)+
  geom_vline(xintercept=0,color="grey",lty=2)+
  geom_point_rast(data=results_aux_promoter_wide_melt_both[TIR1=="Tir1_variant_F79G"  & significance=="Neither"], aes(fill=significance),pch=21,color="black",alpha=.3,raster.dpi=600)+
  geom_point_rast(data=results_aux_promoter_wide_melt_both[TIR1=="Tir1_variant_F79G"  & significance!="Neither"], aes(fill=significance),pch=21,color="black",alpha=.7,raster.dpi=600) +  
  geom_abline(intercept=0,slope=1,lty=2,col="dark gray")+ 
 # geom_abline(intercept=0,slope=-1,lty=2,col="dark gray")+
   ylab("Diffential TIR1[F79G] effect (log2 fold-change)")+
  xlab("TIR1[F79] effect (log2 fold-change)")+ 
  scale_fill_manual(values=colors)+
  scale_color_manual(values=colors)+
  xlim(axis_lims[1],axis_lims[2])+ylim(axis_lims[1],axis_lims[2])+
...
colors = c("black","#FCEA64","#98cF6F")
names(colors) = c("","Different in eif3Bp.1xAID","Different in eft3p.3xAID")

r_temp = results_aux_promoter_wide_melt_both[TIR1=="Tir1_promoter_eif3Bp.1xAID",]
lm_res = glm(value.logFC~Tir1_promoter_wo.0xAID.logFC,
             r_temp,
             family=stats::gaussian(link="identity"))
r_temp$residual = residuals(lm_res)
r_temp[significance%in%c("Both","eft-3"), significance:= ""]
r_temp[significance%in%c("eif3Bp.1xAID","eft3p.3xAID"), significance:= paste("Different in",significance)]

  
axis_lims = range(c(results_aux_promoter_wide_melt_both$Tir1_promoter_wo.0xAID.logFC,results_aux_promoter_wide_melt_both$Tir1_promoter_wo.0xAID.logFC+results_aux_promoter_wide_melt_both$value.logFC))

cr =cor(r_temp$Tir1_promoter_wo.0xAID.logFC,r_temp[,Tir1_promoter_wo.0xAID.logFC+value.logFC])
g_promoter=ggplot(r_temp,aes(Tir1_promoter_wo.0xAID.logFC,Tir1_promoter_wo.0xAID.logFC+value.logFC,color=significance)) + 
   geom_smooth(method="lm",se=F,aes(color=NULL),color="#537D5D")+
  geom_hline(yintercept=0,color="grey",lty=2)+
  geom_vline(xintercept=0,color="grey",lty=2)+
  geom_point_rast(data=r_temp[significance=="Neither"],aes(fill=significance),pch=21,color="black",alpha=.3,raster.dpi=600) +  
  ...
  geom_abline(intercept=0,slope=1,lty=2,col="dark gray")+ 
 # geom_abline(intercept=0,slope=-1,lty=2,col="dark gray")+
  xlab("eft3p::TIR1[F79] effect (log2 fold-change)")+
  ylab("eif3.Bp::TIR1[F79] effect (log2 fold-change)")+ 
  scale_fill_manual(values=colors)+
  scale_color_manual(values=colors)+
  xlim(axis_lims[1],axis_lims[2])+ylim(axis_lims[1],axis_lims[2])+
  annotate("text",label=paste0("rho=",round(cr,2)),x=axis_lims[1]*3/4,y=axis_lims[2]*3/4)+
...
r_temp = results_aux_promoter_wide_melt_both[TIR1=="Tir1_promoter_eft3p.3xAID" ,]
lm_res = glm(value.logFC~Tir1_promoter_wo.0xAID.logFC,
             r_temp,
             family=stats::gaussian(link="identity"))
r_temp$residual = residuals(lm_res)
...
cr =cor(r_temp$Tir1_promoter_wo.0xAID.logFC,r_temp[,Tir1_promoter_wo.0xAID.logFC+value.logFC])
g_degron=ggplot(r_temp,aes(Tir1_promoter_wo.0xAID.logFC,Tir1_promoter_wo.0xAID.logFC+value.logFC,color=significance)) + 
  geom_smooth(method="lm",se=F,aes(color=NULL),color="#537D5D")+
  geom_hline(yintercept=0,color="grey",lty=2)+
  geom_vline(xintercept=0,color="grey",lty=2)+
  geom_point_rast(data=r_temp[significance=="Neither"],aes(fill=significance),pch=21,color="black",alpha=.3,raster.dpi=600) +  
  geom_point_rast(data=r_temp[significance!="Neither"],aes(fill=significance),pch=21,color="black",alpha=1,raster.dpi=600) +  
  geom_abline(intercept=0,slope=1,lty=2,col="dark gray")+ 
 # geom_abline(intercept=0,slope=-1,lty=2,col="dark gray")+
   xlab("daf-2::1xAID effect (log2 fold-change)")+
  ylab("daf-2::3xAID effect (log2 fold-change)")+ 
...
if (0){
r_temp = results_aux_promoter_wide_melt_both


r_temp[,significance := ifelse(Tir1_promoter_wo.0xAID.padj<sig&  abs(Tir1_promoter_wo.0xAID.logFC) >= mag_cutoff , "F79",
                                                                 ifelse(Tir1_variant_F79A.padj<sig & abs(Tir1_variant_F79A.logFC)>=mag_cutoff &
                                                                        Tir1_variant_F79G.padj<sig & abs(Tir1_variant_F79G.logFC)>=mag_cutoff,"Different in both F79G and F79A",
                                                                        ifelse( Tir1_variant_F79G.padj<sig & abs(Tir1_variant_F79G.logFC)>=mag_cutoff,"Different in F79G",
                                                                        ifelse( Tir1_variant_F79A.padj<sig & abs(Tir1_variant_F79A.logFC)>=mag_cutoff,"Different in F79A",
                                                                               "Neither"))))]


axis_lims = range(c(r_temp$Tir1_variant_F79A.logFC+r_temp$Tir1_variant_none.logFC,
                    r_temp$Tir1_variant_F79G.logFC+r_temp$Tir1_variant_none.logFC))

colors = c("grey","black","#FCEA64","#98cF6F","lightgreen")
names(colors) = c("Neither","F79","Different in F79A","Different in F79G","Different in both F79G and F79A")


cr =cor(r_temp$Tir1_variant_F79A.logFC+r_temp$Tir1_variant_none.logFC,r_temp$Tir1_variant_F79G.logFC+r_temp$Tir1_variant_none.logFC)
g_ag=ggplot(r_temp,aes(Tir1_variant_F79A.logFC+Tir1_variant_none.logFC,Tir1_variant_F79G.logFC+Tir1_variant_none.logFC,color=significance)) + 
  geom_smooth(method="lm",se=F,aes(color=NULL),color="#537D5D")+
  geom_hline(yintercept=0,color="grey",lty=2)+
  geom_vline(xintercept=0,color="grey",lty=2)+
  geom_point_rast(data=r_temp[significance=="Neither"],aes(fill=significance),pch=21,color="black",alpha=.3,raster.dpi=600) +  
...
ggpubr::ggarrange(plotlist=list(g_promoter,g_degron),nrow=1,ncol=2)
ggsave("TIR1_auxin_dependent_effects_redux_considering_compound_effect_promoter_and_degron.pdf",width=10,height=5)
...
if (0){ #old bad way of doing it
tmp = results_aux_other_variants_vs_F79_wide


results_aux_ontarget_wide_all3_melt = melt(tmp,id.vars=c("GeneName","AuxinType_Aux.logFC"),measure.vars=c("AuxinType_5ad.logFC","AuxinType_5ph.logFC","AuxinType_Aux.logFC"))

results_aux_ontarget_wide_all3_melt[,value := value]  #because the regression is set up relative to F79 and we need to flip the signs so the effects are WT->F79

results_aux_ontarget_wide_all3_melt[,compound:=substring(as.character(variable),11,nchar(as.character(variable))-6)]

results_aux_ontarget_wide_all3_melt_padj=
melt(tmp,id.vars=c("GeneName","AuxinType_Aux.padj"),measure.vars=c("AuxinType_5ad.padj","AuxinType_5ph.padj","AuxinType_Aux.padj"))
results_aux_ontarget_wide_all3_melt_padj[,compound:=substring(as.character(variable),11,nchar(as.character(variable))-5)]

results_aux_ontarget_wide_all3_melt_both = merge(results_aux_ontarget_wide_all3_melt,results_aux_ontarget_wide_all3_melt_padj,by=c("GeneName","compound"),suffixes=c(".logFC",".padj"))

results_aux_ontarget_wide_all3_melt_both[,significance := ifelse(AuxinType_Aux.padj<.01 & value.padj<.01 & 
                                                                   abs(AuxinType_Aux.logFC) >= 1 & abs(value.logFC) >= 1,
                                                                 "Both",
                                                                 ifelse(AuxinType_Aux.padj<.01 & abs(AuxinType_Aux.logFC)>=1,"Aux",
                                                                        ifelse(value.padj<.01 & abs(value.logFC)>=1,compound,
                                                                               "Neither")))]
colors = c("black","#64C5EB","#E84D8A")
names(colors) = c("","Different in 5ad","Different in 5ph")

sig_genes =  tmp[AuxinType_5ad.logFC < .01 | AuxinType_5ph.padj < .01 | AuxinType_Aux.padj<.01,GeneName]
results_aux_ontarget_wide_all3_melt_both_sig = results_aux_ontarget_wide_all3_melt_both[compound!="Aux"];#[GeneName %in% sig_genes,]
axis_lims = range(c(results_aux_ontarget_wide_all3_melt_both_sig$AuxinType_Aux.logFC,results_aux_ontarget_wide_all3_melt_both_sig$value.logFC))
#unique(res_l_a[,.(significance,colors[significance])])

r_temp = results_aux_ontarget_wide_all3_melt_both_sig[compound=="5ad",]
lm_res = glm(value.logFC~AuxinType_Aux.logFC,
             r_temp,
             family=stats::gaussian(link="identity"))
r_temp$residual = residuals(lm_res)
r_temp[significance%in%c("Both","Aux"), significance:= ""]
r_temp[significance%in%c("5ad","5ph"), significance:= paste("Different in",significance)]

axis_lims = range(c(results_aux_ontarget_wide_all3_melt_both_sig$AuxinType_Aux.logFC,results_aux_ontarget_wide_all3_melt_both_sig$variable.logFC))

cr =cor(r_temp$AuxinType_Aux.logFC,r_temp[,value.logFC])
g_a=ggplot(r_temp,aes(AuxinType_Aux.logFC,value.logFC,color=significance)) + 
   geom_smooth(method="lm",se=F,aes(color=NULL),color="#537D5D")+
  geom_hline(yintercept=0,color="grey",lty=2)+
  geom_vline(xintercept=0,color="grey",lty=2)+
  geom_point_rast(data=r_temp[significance=="Neither"],aes(fill=significance),pch=21,color="black",alpha=.3,raster.dpi=600) +  
  ...
  geom_abline(intercept=0,slope=1,lty=2,col="dark gray")+ 
 # geom_abline(intercept=0,slope=-1,lty=2,col="dark gray")+
  xlab("K-NAA effect (log2 fold-change)")+
  ylab("5-Ad-IAA effect (log2 fold-change)")+ 
  scale_fill_manual(values=colors)+
  scale_color_manual(values=colors)+
  xlim(axis_lims[1],axis_lims[2])+ylim(axis_lims[1],axis_lims[2])+
  annotate("text",label=paste0("rho=",round(cr,2)),x=axis_lims[1]*3/4,y=axis_lims[2]*3/4)+
...
r_temp = results_aux_ontarget_wide_all3_melt_both_sig[compound=="5ph",]
lm_res = glm(value.logFC~AuxinType_Aux.logFC,
             r_temp,
             family=stats::gaussian(link="identity"))
r_temp$residual = residuals(lm_res)
...
cr =cor(r_temp$AuxinType_Aux.logFC,r_temp[,value.logFC])
g_g=ggplot(r_temp,aes(AuxinType_Aux.logFC,value.logFC,color=significance)) + 
   geom_smooth(method="lm",se=F,aes(color=NULL),color="#537D5D")+
  geom_hline(yintercept=0,color="grey",lty=2)+
  geom_vline(xintercept=0,color="grey",lty=2)+
  geom_point_rast(data=r_temp[significance=="Neither"],aes(fill=significance),pch=21,color="black",alpha=.3,raster.dpi=600) +  
 ...
 # geom_abline(intercept=0,slope=-1,lty=2,col="dark gray")+
  xlab("K-NAA effect (log2 fold-change)")+
  ylab("5-Ph-IAA effect (log2 fold-change)")+ 
  scale_fill_manual(values=colors)+
  scale_color_manual(values=colors)+
  xlim(axis_lims[1],axis_lims[2])+ylim(axis_lims[1],axis_lims[2])+
  annotate("text",label=paste0("rho=",round(cr,2)),x=axis_lims[1]*3/4,y=axis_lims[2]*3/4)+
...
ggpubr::ggarrange(plotlist=list(g_a,g_g),nrow=1,ncol=2)
ggsave("TIR1_auxin_dependent_effects_redux_considering_compound_effect=compund_terms.pdf",width=10,height=5)
}
```
...
```{r}
#recalc the effects of double of each variant with auxin
#this code has been replaced by code in "analyze_daf2_transcriptomics", replicated below
if (0){
ref_group = "N2"
annots2 = annotations_aie[Biological_replicate==1 & Auxin==T & Day==1,]
genes_to_exclude = c("TIR1-DERNBERG")
counts2 = counts[ rownames(counts)%!in%genes_to_exclude,annots2$Sample]
nf2 <- calculateSumFactors(counts2)
...
#SEPARATING CONTROLS
#BUT THIS IS A FLAWED METHOD BECAUSE some effects of the compounds are eliminated by the presence of TIR1, and so
#we can't use the compound as control.  this accounted for better by the multiple regression model, above
auxin_types = unique(annotations_aie$AuxinType)
ref="wo"
auxin_types = auxin_types[auxin_types!=ref]
deseq_res2 = rbindlist(lapply(unique(annotations_aie$AuxinType),function(aux_type){
  annots_all = annotations_aie[ Day == 1 & daf2AID>=1 & Auxin == T & AuxinType==aux_type,]
  strains = unique(annots_all$Strain)
  rbindlist(lapply(strains,function(strain){
  
    print(paste(aux_type,strain))
    annots2 = rbind(annots_all[Strain==strain,],
                   annotations_aie[ Day == 1 & daf2AID>=1 & Auxin ==F &AuxinType==ref & Strain == strain,])
    annots2[,Tir1_type := ifelse(Promoter=="wo","none",paste(Promoter,"::",Tir1,";",daf2AID,"xAID",sep=""))]
    annots2[,Tir1_type_auxin := paste(Tir1_type,ifelse(Auxin,paste0("+",AuxinType),""))]
    annots2$Tir1_type_f = relevel(factor(annots2$Tir1_type_auxin),ref=unique(annots2[Auxin==F,Tir1_type_auxin]))
    print(annots2)
    #browser()
    daf2_effect_counts = counts[grepl("WB",rownames(counts)),annots2$Sample]
   ...
   # nf = normalizationFactor(counts_filtered)
    #deseq_res = runDESeq(counts_filtered,annots2,nf,~Tir1_type_f)
    
    nf2 <- calculateSumFactors(daf2_effect_counts)
    deseq_res<- DESeqDataSetFromMatrix(countData = daf2_effect_counts,
                                     colData = annots2,
                                     design = ~ Tir1_type_f)
    sizeFactors(deseq_res) <- nf2 # use scran normalization
    deseq_res <- estimateDispersions(deseq_res) # estimate overdispersion parameter
    deseq_res <- nbinomWaldTest(deseq_res) # run NB GLM and do Wald Test on coefficients
...
results_2_individual_aux_effects_long = deseq_res2[covariate != "Intercept",]
results_2_individual_aux_effects_long[,group:=substring(covariate,nchar("Tir1_type_f_")+1,lapply(gregexpr("_vs",covariate),function(x)x[1]-6))]
results_2_individual_aux_effects_long$Auxin=T

setkey(ind2e_mapping,TIR1_type)
results_2_individual_aux_effects_long[,label :=ind2e_mapping[group,label]]
results_2_individual_aux_effects_long$GeneSymbol = geneid[results_2_individual_aux_effects_long$GeneName,GeneSymbol]
results_2_individual_aux_effects_long[,sig:=padj<.001 & abs(log2FoldChange)>1]
results_2_individual_aux_effects_long = results_2_individual_aux_effects_long[!is.na(sig),]
results_2_individual_aux_effects_long[,covariate:=NULL]
results_2_individual_aux_effects_long[,baseMean :=NULL]
results_2_individual_aux_effects_long[,stat     :=NULL]
#fwrite(results_2_individual_aux_effects_long,"Supplementary table - activated TIR1;daf-2 effects.csv.gz",row.names=F)
}
...
```{r,fig.width=15,fig.height=5}
#compare 'on-target" effects as analyzed using single or double regression models
if (0){
#effect of activated TIR1 compared using single regression models
seperate_regressions = results_2_individual_aux_effects_long[grepl("1x",label) & grepl("eft3p",label),]
seperate_regressions$TIR1 = seperate_regressions[,substring(label,13,16)]                             
seperate_regressions[,TIR1 := ifelse(grepl(")",TIR1),"F79",TIR1)]   
seperate_regressions$model = "single";
seperate_regressions_wide = dcast(seperate_regressions,GeneName~TIR1,value.var="log2FoldChange")
#make everything relative to F79F, which is how the cross model is set up
seperate_regressions_wide[,F79A:=F79A -F79]
seperate_regressions_wide[,F79G:=F79G -F79]
seperate_regressions_wide_melt = melt(seperate_regressions_wide,id.vars="GeneName",variable.name="TIR1",value.name="log2FoldChange")
seperate_regressions_wide_melt_both = merge(seperate_regressions,seperate_regressions_wide_melt,by=c("GeneName","TIR1"),suffixes=c(".variant_relative",".variant_absolute"))
seperate_regressions_wide_melt_both[,log2FoldChange:=log2FoldChange.variant_absolute]

#effect of activated TIR1 compared using a double model
oduble_model_long = results_aux_other_variants_vs_F79_long[grepl("Tir1",group),]
oduble_model_long$TIR1 = oduble_model_long[,substring(group,14,17)]                             
oduble_model_long[,TIR1 := ifelse(TIR1=="none","F79",TIR1)]                               
oduble_model_long[,log2FoldChange := -log2FoldChange]   
oduble_model_long$model = "cross";

compound_results_2_wide = dcast(compound_results_2_long,GeneName~label,value.var="log2FoldChange")


all_reg = rbind(seperate_regressions_wide_melt_both,cross_model_long,fill=T)


all_reg_wide_logFC = dcast(all_reg,GeneName+TIR1~model,value.var = "log2FoldChange")
all_reg_wide_pv = dcast(all_reg,GeneName+TIR1~model,value.var = "padj")
all_reg_wide_both = merge(x=all_reg_wide_logFC,y=all_reg_wide_pv,by=c("TIR1","GeneName"),suffixes=c(".logFC",".padj"))
all_reg_wide_both[,sig:=ifelse(cross.padj < .01 | single.padj < .01,"sig","")]
all_reg_wide_both_with_compound = merge(all_reg_wide_both,compound_results_2_wide,by="GeneName",allow.cartesian=TRUE)

res = lapply(unique(all_reg_wide_both$TIR1),function(tir1){
    d = all_reg_wide_both[TIR1==tir1 & sig=="sig",]
    axis_lims = range(c(d$single.logFC,d$cross.logFC),na.rm=T)
    cr = cor(d$single.logFC,d$cross.logFC,use="complete")
    lm_res = lm(d$cross.logFC~d$single.logFC,na.rm=T)
    list(cr = cr,
         lm = lm_res,
         gp = ggplot(d,aes(single.logFC,cross.logFC))+geom_point(alpha=.5)+geom_smooth(method="lm",se=F)+ 
           xlab(paste(tir1,"single"))+
           ylab(paste(tir1,"cross"))+
           xlim(axis_lims)+ylim(axis_lims)+
           annotate("text",label=paste0("rho=",round(cr,2)),x=axis_lims[1]*3/4,y=axis_lims[2]*3/4))
})
ggarrange(plotlist=list(res[[1]]$gp,res[[2]]$gp,res[[3]]$gp),nrow=1,ncol=3)
}
```
...
```{r,fig.width=15,fig.height=5}
if (0){
#compare 'on-target" effects to compound effects
tir1 = "F79"
d=all_reg_wide_both_with_compound[TIR1==tir1 & sig=="sig",]
axis_lims_x = range(c(d$single.logFC),na.rm=T)
axis_lims_y = range(c(d$KNAA),na.rm=T)
cr = cor(d$single.logFC,d$KNAA,use="complete")
gp1=ggplot(d,aes(single.logFC,KNAA))+geom_point(alpha=.5)+geom_smooth(method="lm",se=F)+ 
           xlab(paste(tir1,"single"))+
           ylab(paste(tir1,"KNAA"))+
           xlim(axis_lims_x)+ylim(axis_lims_y)+
           annotate("text",label=paste0("rho=",round(cr,2)),x=axis_lims_x[1]*3/4,y=axis_lims_y[2]*3/4)

tir1 = "F79G"
d=all_reg_wide_both_with_compound[TIR1==tir1 & sig=="sig",]
axis_lims_x = range(c(d$single.logFC),na.rm=T)
axis_lims_y = range(c(d$`5.Ph.IAA`),na.rm=T)
cr = cor(d$single.logFC,d$`5.Ph.IAA`,use="complete")
gp2=ggplot(d,aes(single.logFC,`5.Ph.IAA`))+geom_point(alpha=.5)+geom_smooth(method="lm",se=F)+ 
           xlab(paste(tir1,"single"))+
           ylab(paste(tir1,"5.Ph.IAA"))+
           xlim(axis_lims_x)+ylim(axis_lims_y)+
           annotate("text",label=paste0("rho=",round(cr,2)),x=axis_lims_x[1]*3/4,y=axis_lims_y[2]*3/4)
...
d=all_reg_wide_both_with_compound[TIR1==tir1 & sig=="sig",]
axis_lims_x = range(c(d$single.logFC),na.rm=T)
axis_lims_y = range(c(d$`5.Ad.IAA`),na.rm=T)
cr = cor(d$single.logFC,d$`5.Ad.IAA`,use="complete")
gp3=ggplot(d,aes(single.logFC,`5.Ad.IAA`))+geom_point(alpha=.5)+geom_smooth(method="lm",se=F)+ 
           xlab(paste(tir1,"single"))+
           ylab(paste(tir1,"5.Ad.IAA"))+
           xlim(axis_lims_x)+ylim(axis_lims_y)+
           annotate("text",label=paste0("rho=",round(cr,2)),x=axis_lims_x[1]*3/4,y=axis_lims_y[2]*3/4)

ggarrange(plotlist=list(gp1,gp2,gp3),nrow=1,ncol=3)
}
```
...
tir1 = "F79"
d=all_reg_wide_both_with_compound[TIR1==tir1 & sig=="sig",]
axis_lims = range(c(d$cross.logFC,d$cross.logFC),na.rm=T)
cr = cor(d$cross.logFC,d$KNAA,use="complete")
gp1=ggplot(d,aes(cross.logFC,KNAA))+geom_point(alpha=.5)+geom_smooth(method="lm",se=F)+ 
           xlab(paste(tir1,"cross"))+
           ylab(paste(tir1,"KNAA"))+
           xlim(axis_lims)+ylim(axis_lims)+
           annotate("text",label=paste0("rho=",round(cr,2)),x=axis_lims[1]*3/4,y=axis_lims[2]*3/4)
...
d=all_reg_wide_both_with_compound[TIR1==tir1 & sig=="sig",]
axis_lims = range(c(d$cross.logFC,d$cross.logFC),na.rm=T)
cr = cor(d$cross.logFC,d$`5.Ph.IAA`,use="complete")
gp2=ggplot(d,aes(cross.logFC,`5.Ph.IAA`))+geom_point(alpha=.5)+geom_smooth(method="lm",se=F)+ 
           xlab(paste(tir1,"cross"))+
           ylab(paste(tir1,"5.Ph.IAA"))+
           xlim(axis_lims)+ylim(axis_lims)+
           annotate("text",label=paste0("rho=",round(cr,2)),x=axis_lims[1]*3/4,y=axis_lims[2]*3/4)
...
d=all_reg_wide_both_with_compound[TIR1==tir1 & sig=="sig",]
axis_lims = range(c(d$cross.logFC,d$cross.logFC),na.rm=T)
cr = cor(d$cross.logFC,d$`5.Ad.IAA`,use="complete")
gp3=ggplot(d,aes(cross.logFC,`5.Ad.IAA`))+geom_point(alpha=.5)+geom_smooth(method="lm",se=F)+ 
           xlab(paste(tir1,"cross"))+
           ylab(paste(tir1,"5.Ad.IAA"))+
           xlim(axis_lims)+ylim(axis_lims)+
           annotate("text",label=paste0("rho=",round(cr,2)),x=axis_lims[1]*3/4,y=axis_lims[2]*3/4)
...
if (0){
all_d2 = all_d[quantification_type%in%c("relative_to_WT","TIR1-ambivalent") & !grepl("3xAID",label),]

d_eft3_F79s = all_d2[(experiment=="degron_double_components_compounds"& grepl("eft3p",label) & grepl("F79\\)",label))]

d_eft3_F79As = all_d2[(experiment=="degron_double_components_compounds" & grepl("eft3p",label) & grepl("F79A",label))]

d_eft3_F79Gs = all_d2[(experiment=="degron_double_components_compounds" & grepl("eft3p",label) & grepl("F79G",label))]

#decompose each TIR1 variant's activated effects into individual components

#This is not the correct way to do it, as we do not take into account the statistical significance of differences between the strains,
#nor the differeing effects of compounds

#the correct way to do this is with the double regression model above.
    res = list(unique(d_eft3_F79s[sig==T,GeneName]),
          unique(d_eft3_F79As[sig==T,GeneName]),
          unique(d_eft3_F79Gs[sig==T,GeneName]) )
    print(lapply(res,length))
    names(res)=c("F79","F79A","F79G")
    
    
    ggVennDiagram(res, 
                  label_alpha=0, 
                  set_color = c("black","#6DE1D2","#F75A5A"),
                  label="count", set_size = 3,label_size=4)+
                ggplot2::scale_fill_gradient(low = "white", high = "white")+theme(legend.position="none")+ scale_x_continuous(expand = expansion(mult = 1))
...
```{r,fig.width=12,fig.height=12}
if (0){
com_stats = function(all_d,title){
  bck = unique(c(all_d[experiment=="degron_double_components" & sig,GeneName],
          all_d[experiment=="degron_single_components" & sig,GeneName],
          all_d[experiment=="compounds" & sig,GeneName]));
  fwd = unique(all_d[experiment=="degron_double_components_compounds" & sig,GeneName])
  ot = setdiff(fwd,bck)
  rmain = setdiff(bck,fwd)
  d=data.table(All.activated=length(fwd),On.Target=length(ot),basal=length(bck),only.basal=length(rmain),title=title)
  d$off.target=d$All.activated-d$On.Target
  d[,.(title,On.Target,off.target,All.activated,basal, only.basal)]
}
all_stats = rbind( com_stats(all_d,"All variants"),
      com_stats(d_eft3_F79,"F79F"),
      com_stats(d_eft3_F79A,"F79A"),
      com_stats(d_eft3_F79G,"F79G"))
all_stats$percent_off = 1-all_stats$On.Target/all_stats$All.activated

on_target_genes = function(all_d,background=NULL){
  if (is.null(background)){
    background = all_d;
  }
    bck = unique(c(background[experiment=="degron_double_components" & sig,GeneName],
            background[experiment=="degron_single_components" & sig,GeneName],
            background[experiment=="compounds" & sig,GeneName]));
  
  fwd = unique(all_d[experiment=="degron_double_components_compounds" & sig,GeneName])
  ot = setdiff(fwd,bck)
  list(on.target=ot,off.target=bck)
}

on_targ_genes = list(
      F79F = on_target_genes(d_eft3_F79)$on.target,
      F79A = on_target_genes(d_eft3_F79A)$on.target,
      F79G = on_target_genes(d_eft3_F79G)$on.target)


ggVennDiagram(on_targ_genes, 
              label_alpha=0, 
              set_color = c("blue","red","green"),
              label="count")+
            ggplot2::scale_fill_gradient(low = "white", high = "white")+theme(legend.position="none")+scale_x_continuous(expand = expansion(mult = 1.1)) 

on_targ_genes_common_background = list(
      F79F = on_target_genes(d_eft3_F79,background=all_d)$on.target,
      F79A = on_target_genes(d_eft3_F79A,background=all_d)$on.target,
      F79G = on_target_genes(d_eft3_F79G,background=all_d)$on.target)


ggVennDiagram(on_targ_genes_common_background, 
              label_alpha=0, 
              set_color = c("blue","red","green"),
              label="count")+
            ggplot2::scale_fill_gradient(low = "white", high = "white")+theme(legend.position="none")+scale_x_continuous(expand = expansion(mult = 1.1)) 
...
#this data is all comming from the fits to the original regression model
#y = compound * TIR1 + e 
#regression model
